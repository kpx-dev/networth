...
resource "aws_api_gateway_integration" "get_transactions" {
  rest_api_id = "${aws_api_gateway_rest_api.api.id}"
  resource_id = "${aws_api_gateway_method.get_transactions.resource_id}"
...
// create /api/transactions/{accountID}
resource "aws_api_gateway_resource" "transactions_accountID" {
  rest_api_id = "${aws_api_gateway_rest_api.api.id}"
  parent_id   = "${aws_api_gateway_resource.transactions.id}"
  path_part   = "{accountID}"
}

resource "aws_api_gateway_method" "get_transactions_accountID" {
  rest_api_id   = "${aws_api_gateway_rest_api.api.id}"
  resource_id   = "${aws_api_gateway_resource.transactions_accountID.id}"
  http_method   = "GET"
  authorization = "COGNITO_USER_POOLS"
  authorizer_id = "${aws_api_gateway_authorizer.auth.id}"
}

resource "aws_api_gateway_integration" "get_transactions_accountID" {
  rest_api_id = "${aws_api_gateway_rest_api.api.id}"
  resource_id = "${aws_api_gateway_method.get_transactions_accountID.resource_id}"
  http_method = "${aws_api_gateway_method.get_transactions_accountID.http_method}"
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = "${aws_lambda_function.api.invoke_arn}"
}
